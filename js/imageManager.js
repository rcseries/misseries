...
class ImageManager {
    ...
    static convertirAEnlaceDirecto(url) {
        ...
        switch (servicio.id) {
            case 'drive': return this.convertirDriveUrl(url);
            case 'imgur': return this.convertirImgurUrl(url);
            case 'cloudinary': return url;
    ...
    static generarPlaceholder(titulo = 'Sin Portada', color = '#667eea') {
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="600" viewBox="0 0 400 600">
            ...
            <text x="200" y="370" font-size="22" text-anchor="middle" fill="white" font-weight="bold">${titulo}</text>
    ...
    static obtenerPortada(url, titulo) {
        if (!url || url.trim() === '') {
            return { url: this.generarPlaceholder(titulo), esPlaceholder: true };
        }
        const urlDirecta = this.convertirAEnlaceDirecto(url);
        return { url: urlDirecta, esPlaceholder: false, servicio: this.detectarServicio(url) };
    }
}
